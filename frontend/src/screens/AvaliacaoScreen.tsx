...
export default function AvaliacaoScreen() {
    ...
    const handleAddBateria = async (questionario: string, data_aplicacao: string) => {
        let profissionalSaudeId = null
        const profile = auth.getUserData()
        if (profile && profile.role === 'profissional_saude') {
            profissionalSaudeId = profile.id
        }
    ...
    const fetchAvaliacaoJoin = async () => {
        ...
        try {
            ...
            const response = await fetch(`${baseUrl}/avaliacoes/paciente/${paciente_id}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
    ...
    const handleDeleteAvalicao = async () => {
        ...
        const token = localStorage.getItem("@App:token");
        try {
            const response = await fetch(`${baseUrl}/avaliacoes/${avaliacaoSelecionada}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
        ...
        <AdminLayout>
...

            {
                avaliacoesJoin.map((avaliacaoJoin) => (
                    ...
                    onOpenMedicoModal={() => setBateriaModalOpen(true)}
